...
| 5 | A Twitch clip | oEmbed (new provider) | 1 or 2 | Identified at all — this provider was a hard 422 before. Check the provider name in the log is "Twitch". |
| 6 | A Steam store page for a game | og:title | 1 | Header shows the store page title; `store.steampowered.com` in full, since only `www.` is stripped. |
